...
async def test_concurrent_start_creates_single_run(
    ...
    async def start_run() -> Run:
        # Each concurrent operation uses an independent database session
        # while both compete for the same Redis-backed bench lock.
        async with session_factory() as session:
    ...
    # Verify the durable state after both concurrent attempts finish.
    async with session_factory() as session:
